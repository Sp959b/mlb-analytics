...
def add_watch(pid: int, name: str, season: int, group: str = "hitting") -> None:
    wl = load_watchlist()
    for p in wl["players"]:
        if int(p["id"]) == int(pid) and int(p["season"]) == int(season) and p.get("group") == group:
            return
    wl["players"].append({"id": int(pid), "name": name, "season": int(season), "group": group})
    save_watchlist(wl)
...
def heat_leaderboard(window: int = 7):
    ...
    for p in players:
        ...
        # need game log
        games = eng.get_player_game_log(pid, season, group)
        if not games or len(games) < window:
            rows.append({"name": name, "season": season, "group": group, "score": None, "detail": "not enough games"})
            continue

        # use your engine’s heat score functions if they exist
        if group == "hitting" and hasattr(eng, "hitter_heat_score_z"):
            info = eng.hitter_heat_score_z(games, windows=(window,)).get(window)
            if info:
                score = info["score"]
        ...
        if group == "pitching" and hasattr(eng, "pitcher_heat_score_z"):
            info = eng.pitcher_heat_score_z(games, windows=(window,)).get(window)
            if info:
                score = info["score"]
                comps = info["components"]
...
<div class="p-3 bg-white soft-card mb-3">
  <form class="row g-2 align-items-end" action="/leaderboard/heat" method="get">
    ...
    <div class="col-12 col-md-6 muted small">
      Heat Score = weighted Z-score blend (higher = hotter).
    </div>
  </form>
</div>
{cards if cards else '<div class="p-3 bg-white soft-card muted">Watchlist is empty.</div>'}
"""
...
def player_season(pid: int, group: str = "hitting", season: int = datetime.now().year):
    ...
    if group == "hitting":
        keys = ["gamesPlayed","plateAppearances","atBats","hits","homeRuns","rbi","avg","obp","slg","ops","strikeOuts","baseOnBalls"]
    else:
        keys = ["gamesPlayed","gamesStarted","wins","losses","era","inningsPitched","strikeOuts","whip","homeRuns","baseOnBalls","saves"]

    rows = ""
    for k in keys:
        rows += f"""
<tr><td class="muted">{k}</td><td class="fw-semibold">{st.get(k, "—")}</td></tr>
"""
...
{rows if rows else '<div class="p-3 bg-white soft-card muted">Watchlist is empty.</div>'}
"""
...
def hr_props_leaderboard(window: int = 7, min_pa: int = 20):
    ...
    for p in hitters:
        ...
        games = eng.get_player_game_log(pid, season, "hitting")
        if len(games) < window:
        ...
        # Optional context if your engine has it
        ctx_str = ""
        p_adj = p_season
...
{cards if cards else '<div class="p-3 bg-white soft-card muted">No hitters in watchlist yet.</div>'}
"""
    return layout("HR Props Board", body)
@app.get("/player/{pid}/rolling", response_class=HTMLResponse)
def player_rolling(pid: int, season: int = datetime.now().year, window: int = 7):
    # rolling is easiest for hitters; you can add pitching later the same way
    group = "hitting"
    games = eng.get_player_game_log(pid, season, group) or []
    if not games:
        return layout("Rolling", f"<div class='p-3 bg-white soft-card'>No game log found.</div>")

    # season baseline from season stats (rates like OPS)
    ...
    for w in windows:
        ...
        hr_m  = _mean([_to_int(g.get("homeRuns")) for g in chunk])
        h_m   = _mean([_to_int(g.get("hits")) for g in chunk])
        k_m   = _mean([_to_int(g.get("strikeOuts")) for g in chunk])

        # deltas vs season OPS (simple but useful)
...
<div class="p-3 bg-white soft-card mb-2">
  ...
  <div class="muted small mt-1">Means per game (using game log)</div>
  <div class="mt-2">
    <div>OPS: <strong>{_fmt(ops_m,3)}</strong> <span class="muted"> (Δ vs season: {_fmt_delta(d_ops,3)})</span></div>
...
def player_zscores(pid: int, season: int = datetime.now().year):
    ...
    games = eng.get_player_game_log(pid, season, group) or []
    if len(games) < 5:
        return layout("Z-Scores", "<div class='p-3 bg-white soft-card'>Not enough game log data for Z-scores.</div>")

    # build season distributions (per game)
    dist_ops  = [_to_float(g.get("ops")) for g in games]
    dist_hr   = [_to_int(g.get("homeRuns")) for g in games]
    dist_hits = [_to_int(g.get("hits")) for g in games]
    dist_k    = [_to_int(g.get("strikeOuts")) for g in games]
    ...
    def rolling_mean(key, n):
        chunk = games[:n]
        vals = []
        for g in chunk:
            if key in ("homeRuns","hits","strikeOuts"):
                vals.append(_to_int(g.get(key)))
            else:
    ...
    for w in windows:
        ...
        hr_m  = rolling_mean("homeRuns", w)      # HR per game
        h_m   = rolling_mean("hits", w)          # hits per game
        k_m   = rolling_mean("strikeOuts", w)    # Ks per game
...
def player_hr_prop_today(pid: int, season: int = datetime.now().year, window: int = 14, min_pa: int = 20):
    # Make sure the engine has the functions we need
    missing = []
    if not hasattr(eng, "season_hr_rate_from_season_stats"):
        missing.append("season_hr_rate_from_season_stats")
    if not hasattr(eng, "get_player_game_log"):
        missing.append("get_player_game_log")
    if not hasattr(eng, "hr_binomial_z"):
        missing.append("hr_binomial_z")
    if not hasattr(eng, "hr_props_today_context"):
        missing.append("hr_props_today_context")
    ...
    # Pull last N games from game log
    ...
    def sum_last_n(key: str, n: int) -> int:
        ...
        for g in games[:n]:
            try:
                total += int(float(g.get(key, 0) or 0))
            except Exception:
                total += 0
        return total
    ...
    # Today context (park + opposing SP)
    today = datetime.now().strftime("%Y-%m-%d")
    ctx = None
    try:
        ctx = eng.hr_props_today_context(pid, season, today)
    ...
    # Adjust baseline HR probability using context multipliers
    p_adj = p_season
    if park_mult is not None:
        p_adj *= park_mult
    if sp_mult is not None:
    ...
    # HR heat z-score (binomial z)
    hr_heat_z = eng.hr_binomial_z(hr_win, pa_win, p_adj)

    # --- Build a simple "Today Score" ---
    # You asked: Today score = HR heat + SP + park
    # We'll convert multipliers into small additive boosts:
    #   boost ≈ 2 * (mult - 1)
    def boost_from_mult(mult):
        if mult is None:
            return 0.0
        try:
    ...
    if hr_heat_z is not None:
        today_score = hr_heat_z + sp_boost + park_boost
    ...
    sp_hr9_str = "n/a" if sp_hr9 is None else f"{float(sp_hr9):.2f}"

    venue_name = (ctx or {}).get("venue_name", "n/a")
    park_factor = (ctx or {}).get("park_hr_factor", None)
    park_factor_str = "n/a" if park_factor is None else f"{float(park_factor):.0f}"
...
<div class="row g-3">
  <div class="col-12 col-md-6">
    <div class="card-dark">
      ...
      <div class="d-flex justify-content-between align-items-center">
        <div class="muted">HR {hr_win} / PA {pa_win}</div>
        {heat_badge}
      </div>
  ...
  <div class="col-12 col-md-6">
    <div class="card-dark">
      ...
      <div class="d-flex justify-content-between align-items-center">
        <div class="muted">Heat + SP + Park</div>
        {score_badge}
      </div>
      <div class="muted small mt-2">
  ...
  <div class="col-12">
    <div class="card-dark">
      ...
      <div class="muted small">
        Notes: If there’s no game / no probable pitcher, SP/Park may show as n/a.
      </div>
    </div>
  </div>
</div>
